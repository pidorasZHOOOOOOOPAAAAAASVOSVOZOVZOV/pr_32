    ...
    public class Record
    {
        ...
        public void Save(bool Update = false)
        {
            if (Update == false)
            {
                DBConnection.Connection($"INSERT INTO [dbo].[Record](" +
                    ...
                    $"VALUES(N'{this.Name}', {this.Year}, {this.Format}, {this.Size}, {this.IdManufacturer}, " +
                    $"{this.Price.ToString(System.Globalization.CultureInfo.InvariantCulture)}, {this.IdState}, N'{this.Description}');");
                this.Id = AllRecords().Where(x => x.Name == this.Name && x.Year == this.Year).First().Id;
            }
            else
            {
                DBConnection.Connection($"UPDATE [dbo].[Record] SET " +
                    $"[Name] = N'{this.Name}', [Year] = {this.Year}, [Format] = {this.Format}, " +
                    $"[Size] = {this.Size}, [IdManufacturer] = {this.IdManufacturer}, " +
                    $"[Price] = {this.Price.ToString(System.Globalization.CultureInfo.InvariantCulture)}, " +
                    $"[IdState] = {this.IdState}, [Description] = N'{this.Description}' WHERE [Id] = {this.Id}");
            }
        }
        ...
        public static void Export(string filePath, IEnumerable<Record> records)
        {
            ...
            foreach (var record in records)
            {
                ...
                csv.AppendLine($"\"{record.Id}\";\"{record.Name}\";\"{record.Year}\";\"{format}\";" +
                    $"\"{record.Size}\";\"{manufacturer}\";\"{record.Price.ToString(System.Globalization.CultureInfo.InvariantCulture)}\";" +
